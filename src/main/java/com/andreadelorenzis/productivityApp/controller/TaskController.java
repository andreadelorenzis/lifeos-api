...
public class TaskController {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Invalid input - missing required fields or invalid frequency for habit",
            content = @Content(mediaType = "application/json")),
        @ApiResponse(responseCode = "404", description = "Goal or Frequency not found",
            content = @Content(mediaType = "application/json"))
    })
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved tasks",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = TaskResponseDTO.class))),
        @ApiResponse(responseCode = "404", description = "Goal not found when filtering by goalId",
            content = @Content(mediaType = "application/json"))
    })
    public ResponseEntity<List<TaskResponseDTO>> listTasks(
            @Parameter(description = "Filter by task type: 'habit' or 'one-time'")
            @RequestParam(required = false) String type,
            @Parameter(description = "Filter tasks by goal ID")
            @RequestParam(required = false) Long goalId,
            @Parameter(description = "Search tasks by name (partial match)")
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "400", description = "Invalid input",
            content = @Content(mediaType = "application/json")),
        @ApiResponse(responseCode = "404", description = "Task, Goal, or Frequency not found",
            content = @Content(mediaType = "application/json"))
    })
